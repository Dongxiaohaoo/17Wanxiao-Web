...
from api.campus_check import (
    get_id_list_v1,
    get_id_list_v2,
    get_customer_type_id,
    get_campus_check_post,
    campus_check_in,
)
    ...
    if not campus_check_config.get("enable"):
        ...
        for index, i in enumerate(id_list):
            ...
            campus_dict = get_campus_check_post(
                template_id=i["templateid"], custom_rule_id=i["id"], stu_num=user_info["stuNo"], token=token
            )
            # 合并配置文件的打卡信息
...
if __name__ == "__main__":
    user_config_path = os.path.join(os.path.dirname(__file__), "conf-init", "user.json")
